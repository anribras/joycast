        ...
        if type_name == 'TracksItem':
            if not self.track_counts:
                ...
                if not self.track_counts:
                    ...
                    rss_result = rss_url if resp.text != '' else ''
                    with app.app_context():
                        ...
                        category, exists = update_or_create(db.session,
                                                            Category,
                                                            name=cate_name)
                        # if exists:
                        ...
                        album, album_exists = update_or_create(db.session, Album,
                                                               is_existed_keys=['source_id'],
                                                               source_type=0,
                        ...
                        for ch in [*self.category.channel, *detail_info['tags']]:
                            tag, exists = update_or_create(db.session, Tag,
                                                           name=ch, is_channel=True)
                            # if exists:
                            #     db.session.add(tag)
                            #     db.session.flush()
                            temp, exists = update_or_create(db.session, AlbumTagPivot,
                                                            album_id=album.id,
                                                            tag_id=tag.id)
                            # if exists:
                            #     db.session.add(temp)
                        ...
                        for t in item.xima:
                            ...
                            track, exists = update_or_create(db.session, Track,
                                                             ...
                                                             rich_intro=info[
                                                                 'richIntro'] if 'richIntro' in info else '',
                                                             short_intro=info[
                                                                 'shortIntro'] if 'shortIntro' in info else '',
